...
public class RealmController implements Controller {
    ...
    public void reconcileAll(List<AddressSpace> addressSpaces) throws Exception {
        ...
        if (authenticationService.getStatus() == null) {
            log.info("Standard authentication service not configured, not performing any operations");
            return;
        }

        if (authenticationService.getSpec().getRealm() != null) {
            log.info("Standard authentication service overriding realm, not performing any operations");
            return;
        }
