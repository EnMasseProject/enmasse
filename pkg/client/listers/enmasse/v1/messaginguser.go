...
type MessagingUserLister interface {
	...
	List(selector labels.Selector) (ret []*v1.MessagingUser, err error)
...
// List lists all MessagingUsers in the indexer.
func (s *messagingUserLister) List(selector labels.Selector) (ret []*v1.MessagingUser, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.MessagingUser))
	})
	return ret, err
}
...
type MessagingUserNamespaceLister interface {
	...
	Get(name string) (*v1.MessagingUser, error)
...
// List lists all MessagingUsers in the indexer for a given namespace.
func (s messagingUserNamespaceLister) List(selector labels.Selector) (ret []*v1.MessagingUser, err error) {
...
// Get retrieves the MessagingUser from the indexer for a given namespace and name.
func (s messagingUserNamespaceLister) Get(name string) (*v1.MessagingUser, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("messaginguser"), name)
	}
	return obj.(*v1.MessagingUser), nil
}
