...
type MessagingPlanLister interface {
	...
	List(selector labels.Selector) (ret []*v1.MessagingPlan, err error)
...
// List lists all MessagingPlans in the indexer.
func (s *messagingPlanLister) List(selector labels.Selector) (ret []*v1.MessagingPlan, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.MessagingPlan))
	})
	return ret, err
}
...
type MessagingPlanNamespaceLister interface {
	...
	Get(name string) (*v1.MessagingPlan, error)
...
// List lists all MessagingPlans in the indexer for a given namespace.
func (s messagingPlanNamespaceLister) List(selector labels.Selector) (ret []*v1.MessagingPlan, err error) {
...
// Get retrieves the MessagingPlan from the indexer for a given namespace and name.
func (s messagingPlanNamespaceLister) Get(name string) (*v1.MessagingPlan, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("messagingplan"), name)
	}
	return obj.(*v1.MessagingPlan), nil
}
