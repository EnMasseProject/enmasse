...
type MessagingEndpointLister interface {
	...
	List(selector labels.Selector) (ret []*v1.MessagingEndpoint, err error)
...
// List lists all MessagingEndpoints in the indexer.
func (s *messagingEndpointLister) List(selector labels.Selector) (ret []*v1.MessagingEndpoint, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.MessagingEndpoint))
	})
	return ret, err
}
...
type MessagingEndpointNamespaceLister interface {
	...
	Get(name string) (*v1.MessagingEndpoint, error)
...
// List lists all MessagingEndpoints in the indexer for a given namespace.
func (s messagingEndpointNamespaceLister) List(selector labels.Selector) (ret []*v1.MessagingEndpoint, err error) {
...
// Get retrieves the MessagingEndpoint from the indexer for a given namespace and name.
func (s messagingEndpointNamespaceLister) Get(name string) (*v1.MessagingEndpoint, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("messagingendpoint"), name)
	}
	return obj.(*v1.MessagingEndpoint), nil
}
