...
type MessagingAddressLister interface {
	...
	List(selector labels.Selector) (ret []*v1.MessagingAddress, err error)
...
// List lists all MessagingAddresses in the indexer.
func (s *messagingAddressLister) List(selector labels.Selector) (ret []*v1.MessagingAddress, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.MessagingAddress))
	})
	return ret, err
}
...
type MessagingAddressNamespaceLister interface {
	...
	Get(name string) (*v1.MessagingAddress, error)
...
// List lists all MessagingAddresses in the indexer for a given namespace.
func (s messagingAddressNamespaceLister) List(selector labels.Selector) (ret []*v1.MessagingAddress, err error) {
...
// Get retrieves the MessagingAddress from the indexer for a given namespace and name.
func (s messagingAddressNamespaceLister) Get(name string) (*v1.MessagingAddress, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("messagingaddress"), name)
	}
	return obj.(*v1.MessagingAddress), nil
}
