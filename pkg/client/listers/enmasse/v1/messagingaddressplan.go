...
type MessagingAddressPlanLister interface {
	...
	List(selector labels.Selector) (ret []*v1.MessagingAddressPlan, err error)
...
// List lists all MessagingAddressPlans in the indexer.
func (s *messagingAddressPlanLister) List(selector labels.Selector) (ret []*v1.MessagingAddressPlan, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.MessagingAddressPlan))
	})
	return ret, err
}
...
type MessagingAddressPlanNamespaceLister interface {
	...
	Get(name string) (*v1.MessagingAddressPlan, error)
...
// List lists all MessagingAddressPlans in the indexer for a given namespace.
func (s messagingAddressPlanNamespaceLister) List(selector labels.Selector) (ret []*v1.MessagingAddressPlan, err error) {
...
// Get retrieves the MessagingAddressPlan from the indexer for a given namespace and name.
func (s messagingAddressPlanNamespaceLister) Get(name string) (*v1.MessagingAddressPlan, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("messagingaddressplan"), name)
	}
	return obj.(*v1.MessagingAddressPlan), nil
}
