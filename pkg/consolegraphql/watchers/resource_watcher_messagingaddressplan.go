...
type MessagingAddressPlanWatcher struct {
	Namespace string
	cache.Cache
	...
	create          func(*tp.MessagingAddressPlan) interface{}
	update          func(*tp.MessagingAddressPlan, interface{}) bool
	restartCounter  int32
	resyncInterval  *time.Duration
}

func NewMessagingAddressPlanWatcher(c cache.Cache, resyncInterval *time.Duration, options ...WatcherOption) (ResourceWatcher, error) {

	kw := &MessagingAddressPlanWatcher{
		...
		create: func(v *tp.MessagingAddressPlan) interface{} {
			return v
		},
		update: func(v *tp.MessagingAddressPlan, e interface{}) bool {
			if !reflect.DeepEqual(v, e) {
				*e.(*tp.MessagingAddressPlan) = *v
				return true
			} else {
				return false
			}
	...
	if kw.ClientInterface == nil {
		return nil, fmt.Errorf("Client must be configured using the MessagingAddressPlanWatcherConfig or MessagingAddressPlanWatcherClient")
	}
	return kw, nil
}

func MessagingAddressPlanWatcherFactory(create func(*tp.MessagingAddressPlan) interface{}, update func(*tp.MessagingAddressPlan, interface{}) bool) WatcherOption {
	return func(watcher ResourceWatcher) error {
		w := watcher.(*MessagingAddressPlanWatcher)
		w.create = create
		w.update = update
		return nil
	}
...
func MessagingAddressPlanWatcherConfig(config *rest.Config) WatcherOption {
	return func(watcher ResourceWatcher) error {
		w := watcher.(*MessagingAddressPlanWatcher)
...
func MessagingAddressPlanWatcherClient(client cp.EnmasseV1Interface) WatcherOption {
	return func(watcher ResourceWatcher) error {
		w := watcher.(*MessagingAddressPlanWatcher)
		w.ClientInterface = client
		return nil
...
func (kw *MessagingAddressPlanWatcher) Watch() error {
	go func() {
		...
		resource := kw.ClientInterface.MessagingAddressPlans(kw.Namespace)
		log.Printf("MessagingAddressPlan - Watching")
		running := true
		for running {
			err := kw.doWatch(resource)
			if err != nil {
				log.Printf("MessagingAddressPlan - Restarting watch - %v", err)
				atomicInc(&kw.restartCounter)
			} else {
				running = false
			}
		}
		log.Printf("MessagingAddressPlan - Watching stopped")
	}()

	return nil
}

func (kw *MessagingAddressPlanWatcher) AwaitWatching() {
	<-kw.watching
}

func (kw *MessagingAddressPlanWatcher) Shutdown() {
	close(kw.stopchan)
	<-kw.stoppedchan
}

func (kw *MessagingAddressPlanWatcher) GetRestartCount() int32 {
	return atomicGet(&kw.restartCounter)
}

func (kw *MessagingAddressPlanWatcher) doWatch(resource cp.MessagingAddressPlanInterface) error {
	...
	_, err = kw.Cache.Get(cache.PrimaryObjectIndex, "MessagingAddressPlan/", func(obj interface{}) (bool, bool, error) {
	...
	var stale = len(curr)
	log.Printf("MessagingAddressPlan - Cache initialised population added %d, updated %d, unchanged %d, stale %d", added, updated, unchanged, stale)
	...
	for {
		...
		case event, chok := <-ch:
			...
			var err error
			log.Printf("MessagingAddressPlan - Received event type %s", event.Type)
			res, ok := event.Object.(*tp.MessagingAddressPlan)
			if !ok {
		...
		case <-kw.stopchan:
			log.Printf("MessagingAddressPlan - Shutdown received")
			return nil
		}
	}
}
...
func (kw *MessagingAddressPlanWatcher) updateGroupVersionKind(o *tp.MessagingAddressPlan) {
	if o.TypeMeta.Kind == "" || o.TypeMeta.APIVersion == "" {
		o.TypeMeta.SetGroupVersionKind(tp.SchemeGroupVersion.WithKind("MessagingAddressPlan"))
	}
}
