...
type MessagingEndpointWatcher struct {
	Namespace string
	cache.Cache
	...
	create          func(*tp.MessagingEndpoint) interface{}
	update          func(*tp.MessagingEndpoint, interface{}) bool
	restartCounter  int32
	resyncInterval  *time.Duration
}

func NewMessagingEndpointWatcher(c cache.Cache, resyncInterval *time.Duration, options ...WatcherOption) (ResourceWatcher, error) {

	kw := &MessagingEndpointWatcher{
		...
		create: func(v *tp.MessagingEndpoint) interface{} {
			return v
		},
		update: func(v *tp.MessagingEndpoint, e interface{}) bool {
			if !reflect.DeepEqual(v, e) {
				*e.(*tp.MessagingEndpoint) = *v
				return true
			} else {
				return false
			}
	...
	if kw.ClientInterface == nil {
		return nil, fmt.Errorf("Client must be configured using the MessagingEndpointWatcherConfig or MessagingEndpointWatcherClient")
	}
	return kw, nil
}

func MessagingEndpointWatcherFactory(create func(*tp.MessagingEndpoint) interface{}, update func(*tp.MessagingEndpoint, interface{}) bool) WatcherOption {
	return func(watcher ResourceWatcher) error {
		w := watcher.(*MessagingEndpointWatcher)
		w.create = create
		w.update = update
		return nil
	}
...
func MessagingEndpointWatcherConfig(config *rest.Config) WatcherOption {
	return func(watcher ResourceWatcher) error {
		w := watcher.(*MessagingEndpointWatcher)
...
func MessagingEndpointWatcherClient(client cp.EnmasseV1Interface) WatcherOption {
	return func(watcher ResourceWatcher) error {
		w := watcher.(*MessagingEndpointWatcher)
		w.ClientInterface = client
		return nil
...
func (kw *MessagingEndpointWatcher) Watch() error {
	go func() {
		...
		resource := kw.ClientInterface.MessagingEndpoints(kw.Namespace)
		log.Printf("MessagingEndpoint - Watching")
		running := true
		for running {
			err := kw.doWatch(resource)
			if err != nil {
				log.Printf("MessagingEndpoint - Restarting watch - %v", err)
				atomicInc(&kw.restartCounter)
			} else {
				running = false
			}
		}
		log.Printf("MessagingEndpoint - Watching stopped")
	}()

	return nil
}

func (kw *MessagingEndpointWatcher) AwaitWatching() {
	<-kw.watching
}

func (kw *MessagingEndpointWatcher) Shutdown() {
	close(kw.stopchan)
	<-kw.stoppedchan
}

func (kw *MessagingEndpointWatcher) GetRestartCount() int32 {
	return atomicGet(&kw.restartCounter)
}

func (kw *MessagingEndpointWatcher) doWatch(resource cp.MessagingEndpointInterface) error {
	...
	_, err = kw.Cache.Get(cache.PrimaryObjectIndex, "MessagingEndpoint/", func(obj interface{}) (bool, bool, error) {
	...
	var stale = len(curr)
	log.Printf("MessagingEndpoint - Cache initialised population added %d, updated %d, unchanged %d, stale %d", added, updated, unchanged, stale)
	...
	for {
		...
		case event, chok := <-ch:
			...
			var err error
			log.Printf("MessagingEndpoint - Received event type %s", event.Type)
			res, ok := event.Object.(*tp.MessagingEndpoint)
			if !ok {
		...
		case <-kw.stopchan:
			log.Printf("MessagingEndpoint - Shutdown received")
			return nil
		}
	}
}
...
func (kw *MessagingEndpointWatcher) updateGroupVersionKind(o *tp.MessagingEndpoint) {
	if o.TypeMeta.Kind == "" || o.TypeMeta.APIVersion == "" {
		o.TypeMeta.SetGroupVersionKind(tp.SchemeGroupVersion.WithKind("MessagingEndpoint"))
	}
}
