...
func (r *ReconcileIoTInfrastructure) processLoraWanAdapter(ctx context.Context, iotInfra *iotv1.IoTInfrastructure, msgInfra *enmassev1.MessagingInfrastructure, qdrProxyConfigCtx *cchange.ConfigChangeRecorder) (reconcile.Result, error) {
	...
	rc.ProcessSimple(func() error {
		return r.processConfigMap(ctx, adapter.FullName()+"-config", iotInfra, !adapter.IsEnabled(iotInfra), func(infra *iotv1.IoTInfrastructure, configMap *corev1.ConfigMap) error {
			return r.reconcileLoraWanAdapterConfigMap(infra, adapter, configMap, change)
		})
	})
...
func (r *ReconcileIoTInfrastructure) reconcileLoraWanAdapterConfigMap(infra *iotv1.IoTInfrastructure, a adapter, configMap *corev1.ConfigMap, change *cchange.ConfigChangeRecorder) error {
