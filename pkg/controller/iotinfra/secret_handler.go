...
func (s *secretHandler) getInfrastructure() (*iotv1.IoTInfrastructure, error) {

	infra := &iotv1.IoTInfrastructure{}

	// we need to lookup the configuration on every change event
	if err := s.client.Get(context.Background(), client.ObjectKey{Namespace: s.namespace, Name: s.configName}, infra); err != nil {
		return nil, err
	} else {
		return infra, nil
	}
...
func (s secretHandler) allSecrets(meta v1.Object) []string {
	...
	if infra.Spec.InterServiceCertificates != nil {

		if infra.Spec.InterServiceCertificates.SecretCertificatesStrategy != nil {
			result = append(result, infra.Spec.InterServiceCertificates.SecretCertificatesStrategy.CASecretName)
			for _, v := range infra.Spec.InterServiceCertificates.SecretCertificatesStrategy.ServiceSecretNames {
				result = append(result, v)
			}
		}
		if infra.Spec.InterServiceCertificates.ServiceCAStrategy != nil {
			result = append(result,
				nameAuthService+"-tls",
	...
	result = s.addSecretsFromEndpoint(result, infra.Spec.ServicesConfig.DeviceRegistry.Management.Endpoint, "")

	// add all endpoint certificates

	for _, a := range adapters {
		if a.IsEnabled(infra) {
			result = s.addSecretsFromEndpoint(result, a.AdapterConfigProvider(infra).EndpointConfig, a.FullName())
		}
	}
...
func (s secretHandler) addSecretsFromEndpoint(result []string, config iotv1.EndpointConfig, fullName string) []string {
...
func (s secretHandler) common(meta v1.Object, q workqueue.RateLimitingInterface) {
	...
	if util.ContainsString(secrets, meta.GetName()) {
		log.Info("Secret is being referenced by IoTInfrastructure, trigger IoTInfrastructure reconciliation", "namespace", meta.GetNamespace(), "name", meta.GetName())
