...
func createIoTTenantCleanerJob(ctx *finalizer.DeconstructorContext, tenant *iotv1alpha1.IoTTenant, config *iotv1alpha1.IoTConfig) (*batchv1.Job, error) {
	...
	if err := c.Get(ctx.Context, key, job); err != nil {
		...
		// not found
		need, err := reconcileIoTTenantCleanerJob(ctx, job, tenantId, config, tenant)
		if err != nil {
		...
		if !need {
			...
			ctx.Recorder.Eventf(tenant, corev1.EventTypeNormal, EventReasonTenantTermination, "No special cleanup required")
			return nil, nil
		}
		if err := c.Create(ctx.Context, job); err != nil {
			ctx.Recorder.Eventf(tenant, corev1.EventTypeWarning, EventReasonTenantTermination, "Failed to create tenant cleanup job: %s", jobName)
			return nil, errors.Wrap(err, "Failed to create cleanup job")
		}

		ctx.Recorder.Eventf(tenant, corev1.EventTypeNormal, EventReasonTenantTermination, "Created tenant cleanup job: %s", jobName)
	...
	if !need {
		...
		ctx.Recorder.Eventf(tenant, corev1.EventTypeWarning, EventReasonTenantTermination, "Cleanup job exists, but is not needed: %s", jobName)
	...
	if err := c.Update(ctx.Context, job); err != nil {
		ctx.Recorder.Eventf(tenant, corev1.EventTypeWarning, EventReasonTenantTermination, "Failed to update tenant cleanup job: %s", jobName)
		return job, err
	}

	// done

	ctx.Recorder.Eventf(tenant, corev1.EventTypeNormal, EventReasonTenantTermination, "Updated tenant cleanup job: %s", jobName)
	return job, nil
}
	...
	err := install.ApplyJobContainerWithError(job, "cleanup", func(container *corev1.Container) error {
		...
		case iotv1alpha1.DeviceConnectionJdbc:
			...
			if external := config.Spec.ServicesConfig.DeviceConnection.JDBC.Server.External; external != nil {
				...
				ctx.Recorder.Event(tenant, corev1.EventTypeWarning, EventReasonTenantTermination, "Unknown JDBC device connection configuration")
		...
		case iotv1alpha1.DeviceRegistryInfinispan:
			...
			if external := config.Spec.ServicesConfig.DeviceRegistry.Infinispan.Server.External; external != nil {
				...
				ctx.Recorder.Event(tenant, corev1.EventTypeWarning, EventReasonTenantTermination, "Unknown Infinispan device registry configuration")
		...
		case iotv1alpha1.DeviceRegistryJdbc:
			...
			if external := config.Spec.ServicesConfig.DeviceRegistry.JDBC.Server.External; external != nil {
				...
				ctx.Recorder.Event(tenant, corev1.EventTypeWarning, EventReasonTenantTermination, "Unknown JDBC device registry configuration")
