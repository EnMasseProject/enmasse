...
func TestCertNew(t *testing.T) {
	...
	_, err = cc.applyCertSecret(secret, caSecret, config, logrtesting.TestLogger{}, "a", "a.example.com", "b.example.com")
	assert.Nil(t, err)
	...
	info, err := GetCertInfo(secret.Data["tls.crt"], secret.Data["tls.key"])
	assert.Nil(t, err)
