...
func (r *ReconcileMessagingUser) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	reqLogger.V(2).Info("checkFinalizer", "user", user.ObjectMeta)
	...
	reqLogger.V(2).Info("createOrUpdate", "user", user.ObjectMeta)
	rc.Process(func() (reconcile.Result, error) {
	...
	reqLogger.V(2).Info("updateGeneration", "user", user.ObjectMeta)
	rc.ProcessSimple(func() error {
