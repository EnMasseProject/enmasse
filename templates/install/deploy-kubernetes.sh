...
do
    ...
    if [ "$auth_service" == "standard" ]; then
        ...
        httpUrl="https://$(kubectl get service -n ${NAMESPACE} standard-authservice -o jsonpath={.spec.clusterIP}):8443/auth"
        runcmd "kubectl create configmap keycloak-config -n ${NAMESPACE} --from-literal=hostname=standard-authservice.${NAMESPACE}.svc --from-literal=httpUrl=$httpUrl --from-literal=port=5671 --from-literal=caSecretName=standard-authservice-cert" "Create standard authentication service configuration"
    fi
done
