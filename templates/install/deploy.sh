...
while getopts a:de:gm:n:o:u:yvh opt; do
    case $opt in
        a)
...
do
    ...
    if [ "$auth_service" == "standard" ]; then
        ...
        runcmd "kubectl create -n ${NAMESPACE} -f ${RESOURCE_DIR}/standard-authservice/deployment.yaml" "Create standard authservice deployment"
        ...
        if [ -n "$USE_OPENSHIFT" ]; then
            runcmd "kubectl create -n ${NAMESPACE} -f ${RESOURCE_DIR}/standard-authservice/route.yaml" "Create standard authservice route"
        else
            runcmd "kubectl create -n ${NAMESPACE} -f ${RESOURCE_DIR}/standard-authservice/external-lv.yaml" "Create standard authservice external service"
        fi
...
if [ "$MODE" == "singletenant" ]; then
    ...
    runcmd "kubectl create -n ${NAMESPACE} -f $RESOURCE_DIR/resource-definitions/address-space-definitions.yaml" "Create address space definitions"
...
runcmd "kubectl create -n ${NAMESPACE} -f ${RESOURCE_DIR}/address-controller/route.yaml" "Create address controller route"
