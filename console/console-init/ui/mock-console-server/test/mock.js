...
describe("mock", function() {
  ...
  describe("patchAddress", function() {
    it("known_plan", function() {
      ...
      assert.strictEqual(
        result.spec.plan.metadata.name,
  ...
  describe("createAddressSpace", function() {
    it("valid_selfsigned", function() {
      ...
      return mock.whenActive(m).then(() => {
        var result = mock.resolvers.Query.addressSpaces(null, {
          filter:
            "`$.metadata.namespace` = 'app1_ns' AND `$.metadata.name` = 'venus_as1'"
        });
        ...
        assert.strictEqual(
          addressSpace.status.caCertificate &&
            addressSpace.status.caCertificate.length > 0,
          true
        );
      });
    });
    it("valid_certBundle", function() {
      ...
      return mock.whenActive(m).then(() => {
        var result = mock.resolvers.Query.addressSpaces(null, {
          filter:
            "`$.metadata.namespace` = 'app1_ns' AND `$.metadata.name` = 'venus_as2'"
        });
        assert.strictEqual(result.total, 1);
        ...
        assert.strictEqual(
          ...
          addressSpace.spec.endpoints[0].certificate.tlsCert
        );
        assert.strictEqual(
          addressSpace.status.caCertificate === undefined,
          true
        );
      });
    });
    it("valid_openshift", function() {
      ...
      return mock.whenActive(m).then(() => {
        var result = mock.resolvers.Query.addressSpaces(null, {
          filter:
            "`$.metadata.namespace` = 'app1_ns' AND `$.metadata.name` = 'venus_as3'"
        });
        assert.strictEqual(result.total, 1);
        ...
        assert.strictEqual(endpointStatus.servicePorts.length, 3);
        assert.strictEqual(
          endpointStatus.certificate && endpointStatus.certificate.length > 0,
          true
        );
        assert.strictEqual(
          addressSpace.status.caCertificate === undefined,
  ...
  describe("messaging_endpoints", function() {
    it("cluster_only", function() {
      ...
      return mock.whenActive(m).then(() => {
        var result = mock.resolvers.Query.messagingEndpoints(null, {
          filter:
            "`$.metadata.namespace` = 'app1_ns' AND `$.metadata.name` LIKE 'mercury_as1.%'"
        });
        assert.strictEqual(result.total, 1);
        ...
        assert.strictEqual(
          ...
          "messaging-mercury_as1.app1_ns.svc"
        );
    ...
    it("amqps_route_and_cluster", function() {
      ...
      return mock.whenActive(m).then(() => {
        var result = mock.resolvers.Query.messagingEndpoints(null, {
          filter:
            "`$.metadata.namespace` = 'app1_ns' AND `$.metadata.name` LIKE 'mercury_as2.%'"
        });
        ...
        assert.strictEqual(
          routeEndpoint.spec.route.routeTlsTermination,
          "passthrough"
        );
        ...
        assert.strictEqual(
          routeEndpoint.status.host,
          "messaging-mercury_as2.app1_ns.apps-crc.testing"
        );
    ...
    it("amqp-wss_route_and_cluster", function() {
      ...
      return mock.whenActive(m).then(() => {
        var result = mock.resolvers.Query.messagingEndpoints(null, {
          filter:
            "`$.metadata.namespace` = 'app1_ns' AND `$.metadata.name` LIKE 'mercury_as3.%'"
        });
        assert.strictEqual(result.total, 2);
        ...
        assert.strictEqual(
          routeEndpoint.status.host,
          "messaging-wss-mercury_as3.app1_ns.apps-crc.testing"
        );
        assert.strictEqual(routeEndpoint.status.ports.length, 1);
    ...
    it("two_routes_and_cluster", function() {
      ...
      return mock.whenActive(m).then(() => {
        var result = mock.resolvers.Query.messagingEndpoints(null, {
          filter:
            "`$.metadata.namespace` = 'app1_ns' AND `$.metadata.name` LIKE 'mercury_as4.%'"
        });
    ...
    it("amqps_route_with_host_and_cluster", function() {
      ...
      return mock.whenActive(m).then(() => {
        var result = mock.resolvers.Query.messagingEndpoints(null, {
          filter:
            "`$.metadata.namespace` = 'app1_ns' AND `$.metadata.name` LIKE 'mercury_as5.%' AND `$.status.type` = 'route'"
        });
        assert.strictEqual(result.total, 1);
    ...
    it("loadbalancer_and_cluster", function() {
      ...
      return mock.whenActive(m).then(() => {
        var result = mock.resolvers.Query.messagingEndpoints(null, {
          filter:
            "`$.metadata.namespace` = 'app1_ns' AND `$.metadata.name` LIKE 'mercury_as6.%' AND `$.status.type` = 'loadBalancer'"
        });
        assert.strictEqual(result.total, 1);
        ...
        assert.strictEqual(
          loadbalancerEndpoint.status.ports[0].protocol,
          "amqps"
        );
      });
    });
