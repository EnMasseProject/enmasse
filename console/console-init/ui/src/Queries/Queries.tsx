...
export const RETURN_ALL_ADDRESS_SPACES = (
  ...
  if (filter_Names && filter_Names.length > 0) {
    if (filter_Names.length > 1) {
      if (filter_Names[0].isExact)
        filter += "(`$.metadata.name` = '" + filter_Names[0].value.trim() + "'";
      else
        filter +=
          "(`$.metadata.name` LIKE '" + filter_Names[0].value.trim() + "%'";
      for (let i = 1; i < filter_Names.length; i++) {
        if (filter_Names[i].isExact)
          filter +=
            "OR `$.metadata.name` = '" + filter_Names[i].value.trim() + "'";
        else
          filter +=
            "OR `$.metadata.name` LIKE '" + filter_Names[i].value.trim() + "%'";
      }
      filter += ")";
    } else {
      if (filter_Names[0].isExact)
  ...
  if (filter_NameSpace && filter_NameSpace.length > 0) {
    if (filter_NameSpace.length > 1) {
      ...
      if (filter_NameSpace[0].isExact)
        filter +=
          "`$.metadata.namespace` = '" + filter_NameSpace[0].value.trim() + "'";
      else
        filter +=
          "`$.metadata.namespace` LIKE '" +
          filter_NameSpace[0].value.trim() +
  ...
  if (sortBy) {
    switch (sortBy.index) {
      ...
      case 3:
        break;
      case 4:
        orderByString = "`$.metadata.creationTimestamp` ";
        break;
      default:
        break;
    }
...
export const RETURN_ALL_ADDRESS_FOR_ADDRESS_SPACE = (
  ...
  let filterString = "";
  if (name && name.trim() !== "") {
    filterString += "`$.metadata.name` LIKE '" + name + ".%' AND";
  }
  if (namespace && namespace.trim() !== "") {
    filterString += "`$.metadata.namespace` = '" + namespace + "'";
  }
...
export const RETURN_ADDRESS_LINKS = (
  ...
  if (filterNames && filterNames.length > 0) {
    if (filterNames.length > 1) {
      if (filterNames[0].isExact)
        filterForLink +=
          "(`$.metadata.name` = '" + filterNames[0].value.trim() + "'";
      else
        filterForLink +=
          "(`$.metadata.name` LIKE '" + filterNames[0].value.trim() + "%' ";
      for (let i = 1; i < filterNames.length; i++) {
        if (filterNames[i].isExact)
          filterForLink +=
            "OR `$.metadata.name` = '" + filterNames[i].value.trim() + "'";
        else
          filterForLink +=
            "OR `$.metadata.name` LIKE '" + filterNames[i].value.trim() + "%' ";
      }
      filterForLink += ")";
    } else {
      if (filterNames[0].isExact)
