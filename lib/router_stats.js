...
var defined_outcomes = ['accepted', 'released', 'rejected', 'modified', 'settled', 'presettled', 'undelivered'];
...
function update_outcomes(outcomes, link_stats) {
    defined_outcomes.forEach(function (name) {
        if (link_stats && link_stats[name + 'Count']) outcomes[name] += link_stats[name + 'Count'];
    });
    return outcomes;
}
...
function collect_by_connection(links, connections) {
    links.forEach(function (link) {
        ...
        if (connection) {
            ...
            if (link.linkDir === 'in') {
                ...
                update_outcomes(connection.outcomes.ingress, link);
                connection.messages_in += l.deliveries;
            } else if (link.linkDir === 'out') {
                connection.receivers.push(l);
                update_outcomes(connection.outcomes.egress, link);
        ...
        Promise.all(routers.map(function (router) { return router.get_connections(); })).then(function (connection_results) {
            ...
            Promise.all(routers.map(function (router) { return router.get_links(); })).then(function (results) {
                ...
                results.forEach(function (links) {
                    count_by_address(links, address_stats);
                    collect_by_connection(links, connections);
                });
