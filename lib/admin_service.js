...
function hostport(service_name, defaults) {
    ...
    if (process.env[service_name + '_SERVICE_HOST'] && process.env[service_name + '_SERVICE_PORT']) {
...
function connect(container, service_name, defaults) {
    ...
    if (options.host) {
        ...
        if (options.properties.container_id === undefined) options.properties.container_id = process.env.HOSTNAME;
