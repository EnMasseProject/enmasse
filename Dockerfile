...
ARG version=latest
ENV ARTEMIS_HOME=/opt/apache-artemis-2.1.0-SNAPSHOT PATH=$ARTEMIS_HOME/bin:$PATH VERSION=${version}

ADD ./build/apache-artemis-bin.tar.gz ./artemis-shutdown-hook/build/distributions/artemis-shutdown-hook.tar /opt/

COPY ./activemq-artemis/integration/activemq-amqp-connector/target/artemis-amqp-connector-2.1.0-SNAPSHOT.jar $ARTEMIS_HOME/lib
...
CMD ["/opt/apache-artemis-2.1.0-SNAPSHOT/bin/main", "/opt/apache-artemis-2.1.0-SNAPSHOT/bin/launch.sh"]
