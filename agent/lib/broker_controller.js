...
BrokerController.prototype.create_address = function (a) {
    ...
    if (a.type === 'queue') {
        ...
        return self.broker.createAddress(a.address, {anycast:true}).then(self.broker.createQueue(a.address).then(function () {
            ...
            log.error('[%s] Failed to create address for queue "%s": %s', self.id, a.address, error);
            self.post_event(myevents.address_failed_create(a, error));
        });
    } else {
