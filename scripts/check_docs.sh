...
function grep_check {
  ...
  local description=$2
  local fatalness=${3:-1}
  x=$(grep -i -E -r -Hn "$pattern" --exclude="*.gif" --exclude="*.jar" --exclude="*.png" --exclude="*.svg" --exclude-dir="design" --exclude-dir="contributing" --exclude-dir="build" --exclude-dir="html" documentation)
  if [ -n "$x" ]; then
    echo "$description:"
    echo "$x"
    y=$(echo "$x" | wc -l)
