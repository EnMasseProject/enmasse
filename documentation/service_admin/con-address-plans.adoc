...
In the xref:con-address-space-plans-{context}[] section, the address space plan referenced two address plans: `small-queue` and `small-anycast`. These address plans are stored as `ConfigMaps` and are defined as follows:
...
Up to 10 addresses with this plan can be created.
