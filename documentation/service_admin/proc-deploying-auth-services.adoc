...
[id='deploying-auth-services_{context}']
...
can either be none (allow all), standard (keycloak), or external (not managed by {ProductName}).
...
IP address for the host name; otherwise, use the hostname of the external route. To get the service IP address:
+
[options="nowrap",subs="attributes"]
----
