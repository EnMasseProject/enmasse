...
[id='deploying-single-address-space-{context}']
= (Optional) Deploying {ProductName} to a single namespace

Although {ProductName} designed to manage multiple namespaces, there are cases where deploying to a
single namespace is useful when evaluating {ProductName} without the required privileges.

.Prerequisite

* At least one authentication service must be installed, see xref:deploying-auth-services-{context}[].
