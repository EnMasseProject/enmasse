...
[id='deploying-single-namespace-{manual-install-context}-{context}']
...
* At least one authentication service must be installed. For more information see * xref:deploying-auth-services-{manual-install-context}-{context}[].
