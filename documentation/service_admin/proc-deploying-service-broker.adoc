...
[id='deploying-service-broker_{context}']
...
*NOTE*: To install {ProductName} on OpenShift, you must have cluster-admin access to set up the required roles for delegating authentication to the Kubernetes master; otherwise, you are restricted to a single address space. For more information about how to deploy without cluster-admin access, which will restrict {ProductName} to a single address space, see xref:deploying-single-address-space[].
