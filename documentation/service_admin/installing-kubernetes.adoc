...
in the messaging console, you need to specify the `oauthUrl` setting. To get the service ip:
+
[options="nowrap"]
----
...
kubectl create configmap keycloak-config --from-literal=hostname=standard-authservice --from-literal=port=5671 --from-literal=oauthUrl=https://$AUTH_HOST:$AUTH_PORT/auth --from-literal=caSecretName=standard-authservice-cert
----
