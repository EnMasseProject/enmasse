...
. Create keycloak configuration used by controller and service. If you want to enable OAUTH support
in the messaging console, you need to specify the `oauthUrl` setting. If you are running a local
OpenShift cluster without a public DNS, use the internal service IP for the hostname, otherwise use
the hostname of the external route. To get the service ip:
+
[options="nowrap"]
----
...
oc create configmap keycloak-config --from-literal=hostname=standard-authservice --from-literal=port=5671 --from-literal=oauthUrl=https://$AUTH_HOST:$AUTH_PORT/auth --from-literal=caSecretName=standard-authservice-cert
----
