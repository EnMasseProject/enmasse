...
[options="nowrap"]
----
oc adm add-role-to-user view system:serviceaccount:$(oc project -q):default
----
...
To deploy {ProductName} in production, it is recommended that you have cluster-admin access in order
to setup the required roles for creating namespaces and managing resources in those namespaces. For
evaluation purposes, {ProductName} can also be deployed in a single namespace.

===== Deploying {ProductName} for production
...
===== Deploying {ProductName} for evaluation
...
oc policy add-role-to-user edit system:serviceaccount:enmasse:enmasse-admin
oc policy add-role-to-user edit system:serviceaccount:enmasse:address-space-admin
----
