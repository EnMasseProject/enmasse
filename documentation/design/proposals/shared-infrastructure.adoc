...
* `AuthenticationService`: No significant changes.
...
IoT design is covered in a separate proposal.
...
spec:
  namespaces:
  - "*" # Which namespaces this infra should serve based on list
  namespaceSelector: # Or selector
    type: messaging
  router:
...
The `MessagingPlan` defines limits that map to the routers per vhost policies:
...
metadata:
  name: small
spec:
  namespaces: # A set of namespaces that should be restricted by this plan
    - foo
  namespaceSelector: # Allows specifying a selector for namespaces that should be restriced by this plan
    type: messaging
  capabilities:
    - transactions
  resources:
...
The messaging tenant will create a singleton instance of `MessagingTenant` in a namespace to configure messaging. Creating the MessagingTenant may be optional: the operator can create it with defaults if addresses are created. Defaults are selected based finding infras, plans and authentication services that are configured to serve the namespace.
...
metadata:
  name: small-queue
spec:
  namespaces: # Which namespaces this plan is available
  - "*"
  namespaceSelector: # Or using label selector
    type: messaging
  resources:
    requests:
      queueMemory: 3Mi
...
The messaging-config controller manages the MessagingTenant CR

* Watch MessagingTenant CR
