...
Router - Broker connections can be maintained either by having the operator create and maintain the router -> broker connectors, or use the broker-plugin to create connectors to the routers. The advantage of the former is that we no longer rely on custom plugin code for connections, and that we get more flexibility in choosing topology of connections (i.e. multiple routers can connect to the same broker for better HA).

Status checks of routers and brokers for address spaces are done using the vcabbage AMQP client in the controller-manager.
...
==== Implement messaging-infra controller in controller-manager (large)

* Watch MessagingInfra CR
* Creates router CR to deploy router based on infra config
* Creates broker CRs to deploy broker based on infra config
   * Create router <-> broker connectors
* Process address spaces configured for a particular infra instance (list entries in cache):
   * Modify router CR to include vhost configuration according to address space and plan
   * Modify router CR to include connector information
   * Process addresses configured for address space:
     * Create router address, autolink(queue) for address
     * Create router address and linkroute for forwarders
     * Create broker addresses using CR _or_ AMQP management

==== Implement status-checker in controller-manager (medium)

* Watch MessagingInfra CR
* Requeue messaging infras periodically
* For each infra CR:
  * Query routers and brokers for a snapshot of their applied configuration periodically
  * Build a map of infras and current status
* Expose address space metrics based on status
* Expose address metrics based on status

==== Implement address-space controller in controller-manager (small)

* Watch AddressSpace CR - populates cache
* Requeue address spaces periodically
* Update address space status based on status map (shared between status-checker

==== Implement address controller in controller-manager (small)

* Watch Address CR - populates cache
* Requeue addresses periodically
* Update address status based on status map (shared between status-checker
