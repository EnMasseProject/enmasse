...
Moreover, the IoTConfig resource deploying the protocol adapters is a singleton in the infrastructure namespace. It might be desirable to support multiple instances of this resource, potentially deployed and managed in different namespaces to the messaging namespace and be managed by a separate persona than the messaging service admin. The IoTProject singleton of a namespace can then refer to the iot config for the project in the same way a messaging plan.
