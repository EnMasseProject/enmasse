...
How would one configure an 'address space' without the CRD?! If no configuration is given, a default authentication service and plan will be used (we can allow the service admin to specify defaults).
Explicit configuration of a namespace would be supported through an optional https://github.com/kubernetes-sigs/kubebuilder/issues/1074#issue-502866823[singleton resource]:
...
spec:
  ...
  authenticationService:
    name: myservice
    namespace: custom # Default is enmasse-infra
    overrides: # Not needed anymore? (I.e. define an authentication service in your own namespace!)
status:
  message: "Unknown authenticationservice 'myservice'
```

Then, other parts of the 'address space' with multiple entities, could also be part of the above, or be configured using CRDs.
...
spec:
  ...
  # Only one of 'service', 'route' and 'loadbalancer' may be specified for each endpoint
  service: # Expose as a service for applications on cluster
    cert:
    ports:
    - name: amqp
    - name: amqps
...
status:
  ...
  host: myendpoint.enmasse-infra.svc
  ports:
  - name: amqp
    port: 5672
```
...
```
apiVersion: admin.enmasse.io/v1
kind: MessagingPlan
metadata:
  name: small
spec:
  messagingInfraRef: shared1 // Plans may refer to the same infra - this means messaging with the same plan will run on the same infra
  capabilities:
    - transactions
  resources:
    requests:
