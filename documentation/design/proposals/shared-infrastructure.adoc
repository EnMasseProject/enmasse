...
For broker deployments, the [activemq-artemis-operator](https://github.com/rh-messaging/activemq-artemis-operator) will be used, (and modified to work with our requirements). The resulting changes should be submitted upstream, although short term there may exist a temporary fork in order to make progress.
...
==== Import artemis operator (medium)
...
==== Implement queue/topic/subscription scheduling (medium)
