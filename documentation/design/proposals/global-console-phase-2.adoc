...
The console-server will accept configuration that defines the statistics avaiable for each configured resource type.  This will take the form a list of named PromQL queries that are associated with each resource type (e.g. address).  These PromQL queries will ultimately produce the values that we wish to display on the console.  The idea is to avoid the need for the console UI to compute values itself.  
...
The metrics API will accept a resource type as an argument.  This must refer to a configured resource type.  The response will be the Prometheus results.  The UI will parse the results and use the values to populate the display.

The metrics request API will accept a filter.  This filter will limit the resource instances considered for processing.  This allows the console to limit the metrics query to those items that need to be displayed on screen, thus reducing the data that needs to be transfered between client/server and keeps the UI responsive.

In the first development iteration the UI will poll for query results.  In a later iteration, a watch feature will be introduced that allows the console to avoid the poll.  This will require the introduction of a metric cache.
