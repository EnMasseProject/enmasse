...
automatically assigned the Identity Provider(s) to the Messaging Tenants (selector mechanism).  
...
If exactly one identity provider is configured for the tenant, or the getting started use-case applies, the username my be passed unqualified.
