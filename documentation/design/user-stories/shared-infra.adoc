...
Specify the following in the `MessagingInfra` to select which plan should be the default for that infra. By default,
an unrestricted plan will be used as default.

* If the reference does not resolve, the infra status will contain a message describing the issue.
* A warning status will be set if the union of the `MessagingInfra` and the `MessagingPlan` selector is a disjoint set.
...
Create the following `MessagingAddressPlan` to enforce limits:
