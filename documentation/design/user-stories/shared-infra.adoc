...
To enable messaging in a Kubernetes namespace, a `MessagingProject` resource must be created by the tenant admin.

* The `MessagingProject` may reference a specific `MessagingInfrastructure`, `MessagingPlan` and `IdentityProviders`
...
* The `MessagingProject` must have a name set to `default`
...
To disable messaging for a namespace, delete the `MessagingProject` resource:

```
kubectl delete messagingtenant default -n myapp
```

NOTE: If there are still addresses and endpoints left in the namespace, the `MessagingProject` will have the phase `Terminating` until they are removed.
...
* AddressSpace -> MessagingProject, MessagingConnector and MessagingEndpoint: Script-assisted
