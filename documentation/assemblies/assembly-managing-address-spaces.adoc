...
Address spaces may also be replaced in order to change the plan, endpoints, network policies or
...
<4> The endpoints that must be configured for the `AddressSpace`.  Endpoints can specity either `messaging`, `console` or `mqtt` services. However, the `mqtt` service is supported for the `standard` address space type only. Endpoints may be changed by replacing the address space.
<5> Expose the configuration of an endpoint. Endpoints can be kept internal to the cluster, or exposed using an OpenShift route or a load-balancer service. Expose configuration may be changed by replacing the address space.
