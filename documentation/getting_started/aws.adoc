...
development and experimentation, and should not be considered a
...
First, you must have created an https://aws.amazon.com/ec2/[EC2 instance]. EnMasse runs on OpenShift
and Kubernetes, but this post uses OpenShift purely for convenience. For the required hardware
configuration, see the
https://docs.openshift.org/latest/getting_started/administrators.html#prerequisites[OpenShift
prerequisites]. The installation will be done using https://www.ansible.com[Ansible], so make sure
...
following configuration to a file, that is, `ansible-inventory.txt`:
...
If you do not have a domain with wildcard support, you can replace with
...
this setup, since it is not a production setup, I grant cluster-admin
