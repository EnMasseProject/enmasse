...
The other fields are used by the {ProductName} console UI. Note the annotation `defined-by`, which
points to an infrastructure config that must exist when an address space using this plan is created.
For more information on infrastructure configs, see xref:infrastructure-configuration-{context}[].
