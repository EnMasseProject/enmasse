...
|`globalDLQ` |If set to `true`, the system establishes a fallback global dead letter address: `!!GLOBAL_DLQ`.  The default is `false`. To remove messages from this address and prevent the broker(s) from becoming full, connect a consuming application to the `!!GLOBAL_DLQ` address .
|===
