...
<6> (Optional) Specifies whether address space overrides are allowed to the host name, port number, realm, and certificates. Valid values are `true` or `false`. If not specified, the default value is `false`.
