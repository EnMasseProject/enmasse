...
.Prerequisites
* link:{BookUrlBase}{BaseProductVersion}{BookNameUrl}#iot-installing-services-messaging-iot-iot[The IoT services are installed].
* link:{BookUrlBase}{BaseProductVersion}{BookNameUrl}#iot-creating-project-messaging-iot-iot[An IoT project is created].
* link:{BookUrlBase}{BaseProductVersion}{BookNameUrl}#assembly-iot-creating-device-messaging-iot-iot[An IoT device is created].
...
[options="nowrap",subs="attributes"]
----
export MESSAGING_HOST=$({cmdcli} get -n myapp addressspace iot -o jsonpath='{.status.endpointStatuses[?(@.name=="messaging")].externalHost}')
