...
In the `brokered` address space, the `*` wildcard can be used at the end of an address in
order to match all words after the `/` delimiter. `topic/*` will match `topic/sub` but not `topic/s/sub`.
