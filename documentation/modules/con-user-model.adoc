...
In the `standard` address space, the `*` wildcard can be used at the end of an address. The address `top*`
...
order to match a single word (`+`) or all words (`*`) after the `/` delimiter. `topic/+` will match
topic/sub` but not `topic/s/sub`.  `topic/*` will match `topic/sub` and `topic/s/sub`.
