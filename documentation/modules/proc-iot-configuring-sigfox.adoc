...
.Prerequisites
* link:#installing-services-{context}[The IoT services are installed].
* link:#iot-creating-project-{context}[An IoT project is created].
...
. link:#iot-proc-creating-device-register-new-device-{context}[Register a new device]. Choose a name, 
  for example, `sigfox-backend`.
. link:#iot-proc-creating-device-set-password-{context}[Set up password credentials] for this device (for example `sigfox-user` / `sigfox-password`).

[id='iot-proc-configuring-sigfox-register-sigfox-device-{context}']
== Registering the Sigfox device

.Prerequisites
* link:#iot-proc-configuring-sigfox-registering-backend-gateway-device-{context}[Registered the Sigfox backend as a gateway device].
...
. link:#iot-proc-creating-device-register-new-device-{context}[Register a new device].
+
...
.Prerequisites
* link:#iot-proc-configuring-sigfox-connection-information-{context}[Prepared the Sigfox connection information].
...
.Prerequisites
* link:#iot-proc-creating-new-sigfox-callback-{context}[Created a callback in the Sigfox backend].
