...
`partitions`, which allows a queue to be sharded across multiple brokers for HA and improved performance. Specifying an amount of `broker` resource above 1 will automatically cause a queue to be partitioned.
