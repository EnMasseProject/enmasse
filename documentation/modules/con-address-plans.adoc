...
----
include::../common/small-queue-plan.yaml[]
----
...
In the `standard` address space, address plans for the address type `queue` also supports a field
`partitions` which allows a queue to be sharded accross multiple brokers for HA and performance
benefits, at the cost of loosing message ordering. Specifying an amount of `broker` resource above
1 will automatically cause a queue to be partitioned.
