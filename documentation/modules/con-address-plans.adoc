...
In the xref:con-address-space-plans-{context}[] section, the address space plan references two address plans: `small-queue` and `small-anycast`. These address plans are stored as custom resources and are defined as follows:

[source,yaml,options="nowrap"]
.link:resources/small-queue-plan.yaml[small-queue-plan.yaml]
----
...
[source,yaml,options="nowrap"]
.link:resources/small-anycast-plan.yaml[small-anycast-plan.yaml]
----
