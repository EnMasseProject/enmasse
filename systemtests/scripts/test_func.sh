...
function create_addres_space() {
    ...
    ADDRESS_SPACE_DEF=$2
    TOKEN=$(oc whoami -t)
    curl -k -X POST -H "content-type: application/json" --data-binary @${ADDRESS_SPACE_DEF} -H "Authorization: Bearer ${TOKEN}" https://$(oc get route -o jsonpath='{.spec.host}' restapi)/v1/addressspaces
...
function create_addresses() {
    ...
    ADDRESSES_DEF=$2
    TOKEN=$(oc whoami -t)
    curl -k -X PUT -H "content-type: application/json" --data-binary @${ADDRESSES_DEF} -H "Authorization: Bearer ${TOKEN}" https://$(oc get route -o jsonpath='{.spec.host}' restapi)/v1/addresses/${ADDRESS_SPACE_NAME}
