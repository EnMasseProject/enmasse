...
class ScaleTest extends TestBase implements ITestBaseIsolated {
    ...
    void testNumberOfSupportedAddresses() throws Exception {
        ...
        int iterator = 0;
        int failureThreshold = 12000;

        while (true) {
            log.info("Total operable addresses: {}", operableAddresses);
            try {
                getResourceManager().appendAddresses(false, getAddressArray(addressSpace).toArray(new Address[0]));
                if (iterator % 200 == 0) {
    ...
    // Help methods
    ///////////////////////////////////////////////////////////////////////////////////
    private List<Address> getAddressArray(AddressSpace addressSpace) {
