...
class UserApiTest extends TestBase implements ITestIsolatedStandard {
    ...
    @BeforeAll
    void beforeAll() {
        commonResourcesManager.setReuseAddressSpace();
    }
    ...
    void testUpdateUserPermissionsCRD() throws Exception {
        ...
        assertThat(KubeCMDClient.getUser(kubernetes.getInfraNamespace(), brokered.getMetadata().getName(), cred.getUsername()).getRetCode(), is(true));
        assertTrue(resourcesManager.getUser(brokered, testUser.getSpec().getUsername()).toString().contains(Operation.recv.toString()));
    ...
    void testUpdateUserPermissionsUserAPI() throws Exception {
        ...
        users.put(brokered, testUser);
        testUser = resourcesManager.createOrUpdateUser(standard, testUser);

        AmqpClient client = getAmqpClientFactory().createQueueClient(standard);
        ...
        resourcesManager.createOrUpdateUser(standard, testUser);
    ...
    void testUserWithSimilarNamesAndAlreadyExistingUser() throws Exception {
        ...
        User testUser = resourcesManager.createOrUpdateUser(brokered, cred);

        UserCredentials cred2 = new UserCredentials("user23", "test_user23");
        User testUser2 = resourcesManager.createOrUpdateUser(brokered, cred2);

        users.put(brokered, testUser);
        users.put(brokered, testUser2);

        assertThrows(KubernetesClientException.class, () ->
                kubernetes.getUserClient(brokered.getMetadata().getNamespace()).create(resourcesManager.getUser(brokered, testUser.getSpec().getUsername())));
    }
    ...
    void testCreateUserUppercaseUsername() throws Exception {
        ...
        assertThrows(KubernetesClientException.class, () -> resourcesManager.createOrUpdateUser(brokered, testUser));
    }
    ...
    void testCreateUsersWithSymbolsInVariousPlaces() throws Exception {
        ...
        User testUser = UserUtils.createUserResource(cred)
                ...
                .done();
        users.put(brokered, testUser);
        resourcesManager.createOrUpdateUser(brokered, testUser);
        ...
        User testUser2 = UserUtils.createUserResource(cred2)
                ...
                .endSpec()
                .done();
        assertThrows(KubernetesClientException.class, () -> resourcesManager.createOrUpdateUser(brokered, testUser2));
        ...
        resourcesManager.createOrUpdateUser(brokered, testUser3);
        ...
        resourcesManager.createOrUpdateUser(brokered, testUser4);
        ...
        User testUser5 = UserUtils.createUserResource(cred5)
                ...
                .done();
        assertThrows(KubernetesClientException.class, () ->
                resourcesManager.createOrUpdateUser(brokered, testUser5));
        ...
        resourcesManager.createOrUpdateUser(brokered, testUser6);
    ...
    void testServiceaccountUser() throws Exception {
        ...
        try {
            resourcesManager.createUserServiceAccount(standard, serviceAccount);
            ...
            getClientUtils().assertCanConnect(standard, messagingUser, Collections.singletonList(queue), resourcesManager);
            ...
            getClientUtils().assertCannotConnect(standard, messagingUser, Collections.singletonList(queue), resourcesManager);
        } finally {
