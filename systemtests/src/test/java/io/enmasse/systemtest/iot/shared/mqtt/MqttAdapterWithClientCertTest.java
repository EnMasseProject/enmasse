 ...
 */
class MqttAdapterWithClientCertTest extends TestBase implements ITestIoTShared {
    ...
    void initEnv() throws Exception {
        ...
        if (registryClient == null) {
            registryClient = cleanup(new DeviceRegistryClient(deviceRegistryEndpoint));
        }
        if (credentialsClient == null) {
            credentialsClient = cleanup(new CredentialsRegistryClient(deviceRegistryEndpoint));
        }
        ...
        this.mqttAdapterClient = cleanup(MqttAdapterClient.create(mqttAdapterEndpoint, deviceId, deviceAuthId, sharedIoTResourceManager.getTenantId(), devicePassword));
        ...
        businessApplicationClient = cleanup(getAmqpClientFactory().createQueueClient(addressSpace));
