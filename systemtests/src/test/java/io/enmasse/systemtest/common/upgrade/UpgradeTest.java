...
class UpgradeTest extends TestBase {
    ...
    private void doTestUpgrade(boolean isAnsible) throws Exception {
        ...
        createAddressSpaceCMD(kubernetes.getNamespace(), "brokered", "brokered", "brokered-single-broker", null, "v1alpha1");
        createAddressSpaceCMD(kubernetes.getNamespace(), "standard", "standard", "standard-unlimited-with-mqtt", null, "v1alpha1");
        Thread.sleep(30_000);

        createUserCMD(kubernetes.getNamespace(), "test-brokered", "test", "brokered", "v1alpha1");
        createUserCMD(kubernetes.getNamespace(), "test-standard", "test", "standard", "v1alpha1");
        Thread.sleep(30_000);

        createAddressCMD(kubernetes.getNamespace(), "brokered-queue", "brokered-queue", "brokered", "queue", "brokered-queue", "v1alpha1");
        createAddressCMD(kubernetes.getNamespace(), "brokered-topic", "brokered-topic", "brokered", "topic", "brokered-topic", "v1alpha1");
        createAddressCMD(kubernetes.getNamespace(), "standard-queue", "standard-queue", "standard", "queue", "standard-large-queue", "v1alpha1");
        createAddressCMD(kubernetes.getNamespace(), "standard-queue-xlarge", "standard-queue-xlarge", "standard", "queue", "standard-xlarge-queue", "v1alpha1");
        createAddressCMD(kubernetes.getNamespace(), "standard-queue-small", "standard-queue-small", "standard", "queue", "standard-small-queue", "v1alpha1");
        createAddressCMD(kubernetes.getNamespace(), "standard-topic", "standard-topic", "standard", "topic", "standard-small-topic", "v1alpha1");
        createAddressCMD(kubernetes.getNamespace(), "standard-anycast", "standard-anycast", "standard", "anycast", "standard-small-anycast", "v1alpha1");
        createAddressCMD(kubernetes.getNamespace(), "standard-multicast", "standard-multicast", "standard", "multicast", "standard-small-multicast", "v1alpha1");
        Thread.sleep(30_000);

        waitUntilDeployed(kubernetes.getNamespace());
        Thread.sleep(60_000);
    ...
    private void installEnmasseBundle(Path templateDir, String version) throws Exception {
        ...
        KubeCMDClient.createNamespace(kubernetes.getNamespace());
        KubeCMDClient.switchProject(kubernetes.getNamespace());
        if (version.startsWith("0.26") || version.equals("1.0")) {
            KubeCMDClient.applyFromFile(kubernetes.getNamespace(), Paths.get(templateDir.toString(), "install", "bundles", productName.equals("enmasse") ? "enmasse-with-standard-authservice" : productName));
        } else {
            KubeCMDClient.applyFromFile(kubernetes.getNamespace(), Paths.get(templateDir.toString(), "install", "bundles", productName));
            KubeCMDClient.applyFromFile(kubernetes.getNamespace(), Paths.get(templateDir.toString(), "install", "components", "example-plans"));
            KubeCMDClient.applyFromFile(kubernetes.getNamespace(), Paths.get(templateDir.toString(), "install", "components", "example-authservices", "standard-authservice.yaml"));
        }
        Thread.sleep(60_000);
        waitUntilDeployed(kubernetes.getNamespace());
    }
    ...
    private void installEnmasseAnsible(Path templatePaths, boolean upgrade) throws Exception {
        ...
        List<String> cmd = Arrays.asList("ansible-playbook", ansiblePlaybook.toString(), "-i", inventoryFile.toString(),
                "--extra-vars", String.format("namespace=%s authentication_services=[\"standard\"]", kubernetes.getNamespace()));
    ...
    private void checkImagesUpdated(String version) throws Exception {
        ...
        kubernetes.listPods().stream().filter(pod -> pod.getMetadata().getName().matches("^qdrouterd-.*-0"))
                .forEach(pod -> kubernetes.deletePod(kubernetes.getNamespace(), pod.getMetadata().getName()));
    ...
    protected boolean sendMessage(String addressSpace, AbstractClient client, UserCredentials
        ...
        arguments.put(ClientArgument.BROKER, KubeCMDClient.getMessagingEndpoint(kubernetes.getNamespace(), addressSpace) + ":443");
