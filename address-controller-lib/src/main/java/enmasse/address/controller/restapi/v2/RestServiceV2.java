...
package enmasse.address.controller.restapi.v2;
    ...
    private static Set<DestinationGroup> mapToDestinations(Map<String, Map<String, AddressProperties>> addressMap) {
        return addressMap.entrySet().stream()
                .map(e -> {
                    ...
                    e.getValue().entrySet()
                            .forEach(d -> {
                                groupBuilder.destination(new Destination(d.getKey(), d.getValue().store_and_forward, d.getValue().multicast, Optional.ofNullable(d.getValue().flavor)));
                            });
