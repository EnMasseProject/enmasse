...
public class ConfigSubscriber {
    ...
    private void createBrokers(Collection<IReplicationController> currentBrokers, Collection<Broker> newBrokers) {
        newBrokers.stream()
                .filter(broker -> !currentBrokers.stream().filter(controller -> broker.address().equals(controller.getLabels().get("address"))).findAny().isPresent())
    ...
    private void deleteBrokers(Collection<IReplicationController> currentBrokers, Collection<Broker> newBrokers) {
        currentBrokers.stream()
                .filter(controller -> !newBrokers.stream().filter(broker -> broker.address().equals(controller.getLabels().get("address"))).findAny().isPresent())
